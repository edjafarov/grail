      ...
      function appHandler(renderStuff){
        return function (Handler, state){
          ...
          state.$render = function(){
            ...
            Handler.childContextTypes.doAction = React.PropTypes.func.isRequired;
            Handler.childContextTypes.stores = React.PropTypes.object.isRequired;
            Handler.getChildContext = function() {
              return {doAction:doAction, stores: state.stores};
            },*/
            React.withContext({doAction:doAction, stores: state.stores},renderStuff.bind({Handler:React.createFactory(Handler), stores: state.stores}))

            //renderStuff.call({Handler:React.createFactory(Handler), stores: state.stores});
          }
          //get matched path's
          var urlsMatched = []
      ...
      return {
        ...
        middleware: function(req, res, next){
          ...
          theApp.renderUrl(req, appHandler(function(err){
            ...
            var html = React.renderToStaticMarkup(React.createFactory(Html)({ markup :renderedApp}));
